...
public class RevocableJwtTokenStore extends JwtTokenStore {
    ...
    public OAuth2AccessToken restoreToken(String tokenValue) {
        OAuth2AccessToken storedAccessToken = redisTokenStore.readAccessToken(tokenValue);
        OAuth2Authentication storedAuthentication = redisTokenStore.readAuthentication(tokenValue);
        if (storedAccessToken != null && storedAuthentication != null) {
            return storedAccessToken;
        }
        OAuth2AccessToken jwtAccessToken = super.readAccessToken(tokenValue);
        OAuth2Authentication jwtAuthentication = super.readAuthentication(tokenValue);
        storeAccessToken(jwtAccessToken, jwtAuthentication);
